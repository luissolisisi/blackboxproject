...
////////////eventos de alta usuario///////////////
		$("#frmusuario").submit(function(e) {
			...
			var str = window.location.href;
                        ...
                        console.log($('#conf_password').val());
                        if($('#password_pld').val() != $('#conf_password').val()) {
                            $("#message-error").show();
                            $("#message-error").fadeOut(6000);
                        }else{
		...
		$("#form-subcreditos").submit(function(e) {
			e.preventDefault();
			var formulario = $("#form-subcreditos").serialize();

			$.ajax({
				type: "post",
				url: "<?= base_url('clientes/get_Insert_subcredito') ?>",
				dataType: 'json',
				data: formulario,
