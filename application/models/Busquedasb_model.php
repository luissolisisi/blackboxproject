...
class Busquedasb_model extends CI_Model {

    function __construct() {
        parent::__construct();
        $this->db_pro_a = $this->load->database('prolistas_a', TRUE);
        $this->load->database();
    }
    ...
    public function get_totalBusquedas() {
        ...
        $query = $this->db_pro_a->query("SELECT sum(busqueda) as total from listas_binnacle WHERE id_entidad='" . $entidad . "' AND seccion='Buscar'");
        $row = $query->row();
        return $row->total;
    }

    public function get_nacionales() {
        return $this->db_pro_a->get_where('cat_listas', array('status' => 'active', 'clave_tipo' => 'Nacional'))->result_array();
    }

    public function get_internacionales() {
        $this->db_pro_a->group_by("nombre");
        return $this->db_pro_a->get_where('cat_listas', array('status' => 'active', 'clave_tipo' => 'Internacional'))->result_array();
    }

    public function get_list($id = 0) {
        $result = $this->db_pro_a->get_where('cat_lists', array('id' => $id));
    ...
    public function get_listN($id = 0) {
        $result = $this->db_pro_a->get_where('book_listas', array('id' => $id));
        if ($result->num_rows() > 0) {
            return $result->result();
        }
        return Array();
    ...
    public function buscar_exacta($nombre, $apaterno, $amaterno, $rfc, $curp, $status = "active") {
        ...
        if (!empty($rfc && $curp)) {
            $search = "SELECT * FROM cat_lists where concat(nombre,apaterno,amaterno) = '$fullname' or alias='$fullname1' and rfc = '$rfc' and curp = '$curp' AND
               (id_entidad=1500 or id_entidad=0 OR id_entidad IS NULL OR id_entidad='$id_entidad')";
        } elseif (!empty($curp)) {
            $search = "SELECT * FROM cat_lists where concat(nombre,apaterno,amaterno) = '$fullname' or alias='$fullname1' and curp = '$curp' AND
                (id_entidad=1500 or id_entidad=0 OR id_entidad IS NULL OR id_entidad='$id_entidad')";
        } elseif (!empty($rfc)) {
            $search = "SELECT * FROM cat_lists where concat(nombre,apaterno,amaterno) = '$fullname' or alias='$fullname1'  and rfc = '$rfc'AND
                (id_entidad=1500 or id_entidad=0 OR id_entidad IS NULL OR id_entidad='$id_entidad')";
        } else {
            $search = "SELECT * FROM cat_lists where concat(nombre,apaterno,amaterno) = '$fullname' or alias='$fullname1' AND
                (id_entidad=1500 or id_entidad=0 OR id_entidad IS NULL OR id_entidad='$id_entidad')";
        }
        ...
        if (empty($query->num_rows())) {
            $search = "SELECT * FROM cat_lists where concat(nombre,apaterno,amaterno) = '$fullname' or alias='$fullname1' AND
              (id_entidad=1500 or id_entidad=0 OR id_entidad IS NULL OR id_entidad='$id_entidad')";
            $query = $this->db_pro_a->query($search);
            //$query = $this->db_pro_a->get_where('cat_lists', array('nombre' => $nombre.' '.$apaterno.' '.$amaterno));
            if (empty($query->num_rows())) {
                $search = "SELECT * FROM cat_lists where concat(nombre,apaterno,amaterno) = '$fullname' or alias='$fullname1' AND
                  (id_entidad=1500 or id_entidad=0 OR id_entidad IS NULL OR id_entidad='$id_entidad')";
                $query = $this->db_pro_a->query($search);
                //$query = $this->db_pro_a->get_where('cat_lists', array('nombre' => $apaterno.' '.$amaterno.' '.$nombre ));
            }
        }
    ...
    public function buscar($trimabuscar, $nombre, $apaterno, $amaterno, $rfc = "", $curp = "", $status = "active") {
        ...
        if (!empty($rfc) && !empty($curp)) {
            $search = "SELECT * FROM cat_lists where rfc='" . $rfc . "' AND curp = '" . $curp . "' AND
          (id_entidad=1500 or id_entidad=0 OR id_entidad IS NULL OR id_entidad='$id_entidad')";
        } elseif (!empty($rfc)) {
            $search = "SELECT * FROM cat_lists where rfc='" . $rfc . "'AND
          (id_entidad=1500 or id_entidad=0 OR id_entidad IS NULL OR id_entidad='$id_entidad')";
        } elseif (!empty($curp)) {
            $search = "SELECT * FROM cat_lists where curp='" . $curp . "'AND
    ...
    public function buscarApi($trimabuscar, $nombre, $apaterno, $amaterno, $rfc = "", $curp = "", $status = "active", $idE) {
        ...
        if (!empty($rfc) && !empty($curp)) {
            $search = "SELECT * FROM cat_lists where rfc='" . $rfc . "' AND curp = '" . $curp . "'";
        } elseif (!empty($rfc)) {
            $search = "SELECT * FROM cat_lists where rfc='" . $rfc . "'";
        } elseif (!empty($curp)) {
            $search = "SELECT * FROM cat_lists where curp='" . $curp . "'";
        } else {
            $search = "
    						SELECT *
     						FROM cat_lists
                    ...
                    OR alias='" . $trimabuscar . "'
    	   				  	OR nombre LIKE '%" . $nombre . "%'
    	   				  	AND apaterno = '" . $apaterno . "'
    						    AND amaterno = '" . $amaterno . "') and id_entidad IN (1500," . $idE . ")";
            $search_order = " ORDER BY id DESC;";
            $search .= $search_order;
        }
        ...
        if (empty($query->num_rows())) {
            $search = "
                  SELECT * FROM cat_lists where concat(nombre,apaterno,amaterno) = '$fullname' or alias='$fullname1' and id_entidad IN (1500,$entidad)";
            $query = $this->db_pro_a->query($search);
            if (empty($query->num_rows())) {
                $search = "
                      SELECT * FROM cat_lists where concat(nombre,apaterno,amaterno) = '$fullname' or alias='$fullname1' and id_entidad IN (1500,$entidad)";
...
//                //print_r($this->db_pro_a->last_query());
...
//    		}
    ...
    public function search_rfc($rfc = '') {
        $query = $this->db_pro_a->get_where('cat_lists', array('rfc' => $rfc));
        if ($query->num_rows() > 0) {
            return true;
        } else {
            return false;
    ...
    public function buscar_extendida($trimabuscar, $nombre, $apaterno, $amaterno, $rfc = "", $curp = "", $status = "active") {
        ...
        $search = "
          SELECT *
          FROM cat_lists
          WHERE  (id_entidad=1500 or id_entidad=0 OR id_entidad IS NULL OR id_entidad=" . $id_entidad . ") AND
          (nombre LIKE '%" . $nombre . "%'
    ...
    public function buscar_exacta_m($nombre, $rfc, $status = "active") {
        $id_entidad = $this->session->userdata('entidad');
        if (!empty($rfc)) {
            $search = "SELECT * FROM cat_lists where nombre = '$nombre' or alias='$nombre' and rfc = '$rfc' AND
              (id_entidad=1500 or id_entidad=0 OR id_entidad IS NULL OR id_entidad=$id_entidad)";
        } else {
            $search = "SELECT * FROM cat_lists where (id_entidad=1500 or id_entidad=0 OR id_entidad IS NULL OR id_entidad=$id_entidad)
                and (nombre = '$nombre' or alias='$nombre')";
        }
        $query = $this->db_pro_a->query($search);
        if (empty($query->num_rows())) {
            $query = $this->db_pro_a->get_where('cat_lists', array('nombre' => $nombre));
            if (empty($query->num_rows())) {
                $query = $this->db_pro_a->get_where('cat_lists', array('nombre' => $nombre));
            }
        }
    ...
    public function buscar_exacta_mApi($nombre, $rfc, $status = "active") {
        if (!empty($rfc)) {
            $search = "SELECT * FROM cat_lists where nombre = '$nombre' or alias='$nombre' and rfc = '$rfc'";
        } else {
            $search = "SELECT * FROM cat_lists where nombre = '$nombre'";
        }
        $query = $this->db_pro_a->query($search);
        if (empty($query->num_rows())) {
            $query = $this->db_pro_a->get_where('cat_lists', array('nombre' => $nombre));
    ...
    public function unblock_person($data, $id) {
        $query = $this->db_pro_a->query("SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE COLUMN_NAME = 'related_trade' AND TABLE_NAME = 'cat_lists' AND TABLE_SCHEMA = '" . $this->db_pro_a_g->database . "' ");
        if ($query->num_rows() == 0) {
            $query = $this->db_pro_a_g->query("ALTER TABLE cat_lists ADD related_trade varchar(50)  NULL");
        }

        $query = $this->db_pro_a_g->query("SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE COLUMN_NAME = 'reason' AND TABLE_NAME = 'cat_lists' AND TABLE_SCHEMA = '" . $this->db_pro_a_g->database . "' ");
        if ($query->num_rows() == 0) {
            $query = $this->db_pro_a_g->query("ALTER TABLE cat_lists ADD reason varchar(50)  NULL");
        }

        $this->db_pro_a->where('id', $id);
        $this->db_pro_a->update('cat_lists', $data);

        if ($this->db_pro_a_g->affected_rows() > 0):
            return true;
        endif;
        return false;
    }
    ...
    public function buscar_extendida_m($nombre, $rfc, $status = "active") {
        ...
        FROM cat_lists
        WHERE (id_entidad=1500 OR id_entidad=" . $id_entidad . ") AND
         (nombre LIKE '%" . $nombre . "%' OR alias='" . $nombre . "')";
            $search_order = " ORDER BY id DESC;";
            $search .= $search_order;
        }
        $query = $this->db_pro_a->query($search);
    ...
    public function totalBusquedas_Api($apiK) {
        ...
        $query = $this->db_pro_a->query("SELECT sum(busqueda) as total from listas_binnacle WHERE id_entidad='" . $apiK . "' AND seccion='Buscar API'");
        $row = $query->row();
        return $row->total;
    }

    public function varSesion($apiK) {
        $query = $this->db_pro_a->query("select * from entidades_prolistas where idEntidad='" . $apiK . "'");
    ...
    public function busquedaYotepresto($trimabuscar, $nombre, $apaterno, $amaterno, $rfc) {
        ...
        if (!$rfc == '') {
            $search = "
                  SELECT id,nombre,apaterno,amaterno,pertenece,actividad,fecha,tipo,status,updated_at FROM cat_lists where rfc='$rfc'";
            $query = $this->db_pro_a->query($search);


            if (empty($query->num_rows())) {
                $search2 = "
                  SELECT id,nombre,apaterno,amaterno,pertenece,actividad,fecha,tipo,status,updated_at FROM cat_lists where nombre like '%$nombre%' and apaterno='$apaterno' and amaterno='$amaterno' ";
                $query = $this->db_pro_a->query($search2);
            }
        } else {
            $search2 = "
                  SELECT id,nombre,apaterno,amaterno,pertenece,actividad,fecha,tipo,status,updated_at FROM cat_lists where nombre like '%$nombre%' and apaterno='$apaterno' and amaterno='$amaterno'";
    ...
    public function busquedaYoteprestoM($nombre, $rfc) {
        ...
        if (!$rfc == '') {
            ...
            if (empty($query->num_rows())) {
                $search2 = "
                SELECT id,nombre,apaterno,amaterno,pertenece,actividad,fecha,tipo,status,updated_at FROM cat_lists where nombre LIKE '%$nombre%' or alias='$nombre'";
                $query = $this->db_pro_a->query($search2);
            }
        } else {
            $search2 = "
...
//            $search .= $search_order;
        ...
        $search = "SELECT id,nombre,apaterno,amaterno,pertenece,actividad,fecha,tipo,updated_at,alias,status 
FROM propld_access.cat_lists WHERE MATCH(nombre_completo) AGAINST ('$trimabuscar' IN BOOLEAN MODE)  AND (id_entidad=1500 or id_entidad=" . $entidad . ");";
